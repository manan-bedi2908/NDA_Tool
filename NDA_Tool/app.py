...
with st.sidebar:
    ...
    if projects:
        ...
        for proj_id, proj in projects.items():
            ...
            is_pending = proj_id == pending_delete_id
            ...
            with st.container():
                ...
                with col1:
                    ...
                    # Disable selection if any project is pending deletion to avoid confusion
                    if st.button(button_label, key=f"select_{proj_id}", use_container_width=True, disabled=pending_delete_id is not None):
                        set_current_project(proj_id)
                        st.rerun()
                
                with col2:
                    # Disable other delete buttons while one is pending
                    if st.button("🗑️", key=f"del_{proj_id}", disabled=pending_delete_id is not None):
                        st.session_state["pending_delete_id"] = proj_id
                        st.rerun()
    ...
    # Confirmation UI for pending deletion (MOVED DOWN)
    if pending_delete_id:
        if pending_delete_id in projects:
            with st.container():
                st.warning(f"⚠️ **Confirm Deletion**\n\nAre you sure you want to delete project: **{projects[pending_delete_id]['name']}**?")
                confirm_col1, confirm_col2 = st.columns(2)
                with confirm_col1:
                    if st.button("✅ Confirm Delete", key="confirm_del", type="primary", use_container_width=True):
                        project_name = projects[pending_delete_id]['name']
                        delete_project(pending_delete_id)
                        del st.session_state["pending_delete_id"]
                        st.session_state["delete_success"] = f"✅ Project '{project_name}' deleted successfully"
                        st.rerun()
                with confirm_col2:
